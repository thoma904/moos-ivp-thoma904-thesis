...
/*    FILE: TowObstacleSim.cpp                                   */
/*    DATE: March 2026                                           */
/*    BASED ON: ObstacleSim by Michael Benjamin                  */
/*                                                               */
/*  Splits obstacles into two halves: the first half is          */
/*  designated for the tow (published on TOW_GIVEN_OBSTACLE)     */
/*  and the second half for the vehicle (GIVEN_OBSTACLE).        */
/*****************************************************************/
...
TowObstacleSim::TowObstacleSim()
{
  ...
  m_tow_fill_color  = "yellow";
  m_tow_edge_color  = "orange";
...
bool TowObstacleSim::OnNewMail(MOOSMSG_LIST &NewMail)
{
...
bool TowObstacleSim::OnConnectToServer()
{
   registerVariables();
   return(true);
}
...
bool TowObstacleSim::OnStartUp()
{
  ...
  for(p=sParams.begin(); p!=sParams.end(); p++) {
    ...
    else if((param == "tow_fill_color") && isColor(value))
      handled = setColorOnString(m_tow_fill_color, value);
    else if((param == "tow_edge_color") && isColor(value))
      handled = setColorOnString(m_tow_edge_color, value);
...
void TowObstacleSim::registerVariables()
{
...
bool TowObstacleSim::handleMailNodeReport(string report, string& whynot)
{
...
bool TowObstacleSim::handleMailPointSize(string str)
{
  str = tolower(str);
...
bool TowObstacleSim::handleConfigMinDuration(string sval)
{
  if(!isNumber(sval))
    return(false);
...
bool TowObstacleSim::handleConfigMaxDuration(string sval)
{
  if(!isNumber(sval))
    return(false);
...
bool TowObstacleSim::handleConfigObstacleFile(string filename)
{
...
void TowObstacleSim::handleConfigObstacleDurations()
{
  if(m_max_duration <= 0)
    return;
...
void TowObstacleSim::updateVRanges()
{
...
void TowObstacleSim::updateObstaclesField()
{
  if(!m_reset_pending)
    return;
  ...
  // Recompute the tow/vehicle split
  m_tow_split = m_obstacles.size() / 2;

  // Apply the local simulator viewing preferences
  for(unsigned int i=0; i<m_obstacles.size(); i++) {
    if(i < m_tow_split) {
      // Tow obstacles: use tow-specific colors
      ...
      // Vehicle obstacles: use standard colors
...
void TowObstacleSim::updateObstaclesRefresh()
{
...
//     Notes: Splits obstacles into tow vs vehicle sets.
//            Tow obstacles [0, m_tow_split) -> TOW_GIVEN_OBSTACLE
//            Vehicle obstacles [m_tow_split, size) -> GIVEN_OBSTACLE

void TowObstacleSim::postObstaclesRefresh()
{
  // =================================================
  ...
  // =================================================
  // Part 2: Post ground truth and given obstacles
  // =================================================
  for(unsigned int i=0; i<m_obstacles.size(); i++) {
    string spec = m_obstacles[i].get_spec_pts_label(2);
    string key  = m_obstacles[i].get_label();
    string src  = m_obstacles[i].get_vsource();
    if(src != "")
      spec += ",vsource=" + src;
    if(m_durations[i] >= 0)
    ...
    if(!m_post_points) {
      if(i < m_tow_split) {
	// Tow obstacle
	Notify("TOW_GIVEN_OBSTACLE", spec);
      }
      else {
	// Vehicle obstacle
...
void TowObstacleSim::postObstaclesErase()
{
  for(unsigned int i=0; i<m_obstacles.size(); i++) {
...
//      Note: Tow obstacles -> TRACKED_FEATURE_TOW_<VNAME>
//            Vehicle obstacles -> TRACKED_FEATURE_<VNAME>

void TowObstacleSim::postPoints()
{
  vector<string> vnames = m_ledger.getVNames();
  for(unsigned int i=0; i<vnames.size(); i++) {
    ...
    for(unsigned int j=0; j<m_obstacles.size(); j++) {
      if(m_obstacles[j].dist_to_poly(osx,osy) <= m_sensor_range) {
	for(unsigned int k=0; k<m_rate_points; k++) {
	  double x, y;
	  bool ok = randPointOnPoly(osx, osy, m_obstacles[j], x, y);
	  if(ok) {
	    string key = m_obstacles[j].get_label();
	    string src = m_obstacles[j].get_vsource();
	    ...
	    // Tow obstacles get a separate tracked feature variable
	    if(j < m_tow_split)
	      Notify("TOW_TRACKED_FEATURE_"+uvname, msg);
	    else
...
bool TowObstacleSim::buildReport()
{
  ...
  unsigned int tow_count = m_tow_split;
  unsigned int veh_count = m_obstacles.size() - m_tow_split;

  m_msgs << "================================" << endl;
  m_msgs << "Config (Obstacles) " << endl;
  m_msgs << "  Obstacles: " << uintToString(m_obstacles.size()) << endl;
  m_msgs << "    Tow:     " << uintToString(tow_count) << endl;
  m_msgs << "    Vehicle: " << uintToString(veh_count) << endl;
  ...
  ACTable actab(5);
  actab << "Obs | Assign  | Obs      | Points     | Given ";
  actab << "Key | ment    | Duration | Published  | Published";
  actab.addHeaderLines();

  for(unsigned int i=0; i<m_obstacles.size(); i++)  {
    string key = m_obstacles[i].get_label();
    string assign_str = (i < m_tow_split) ? "tow" : "vehicle";
