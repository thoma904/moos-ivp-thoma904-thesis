...
for ARGI; do
    ...
    elif [ "${ARGI:0:16}" = "--tow_pwt_outer=" ]; then
        TOW_PWT_OUTER="${ARGI#--tow_pwt_outer=*}"
    elif [ "${ARGI:0:16}" = "--tow_pwt_inner=" ]; then
        TOW_PWT_INNER="${ARGI#--tow_pwt_inner=*}"
    elif [ "${ARGI:0:16}" = "--veh_pwt_outer=" ]; then
        VEH_PWT_OUTER="${ARGI#--veh_pwt_outer=*}"
    elif [ "${ARGI:0:16}" = "--veh_pwt_inner=" ]; then
        VEH_PWT_INNER="${ARGI#--veh_pwt_inner=*}"
    elif [ "${ARGI:0:10}" = "--tow_ttc=" ]; then
        TOW_TTC="${ARGI#--tow_ttc=*}"
...
# custom
    ...
    IVARGS+=" --tow_pwt_outer=${TOW_PWT_OUTER} --tow_pwt_inner=${TOW_PWT_INNER} "
    IVARGS+=" --veh_pwt_outer=${VEH_PWT_OUTER} --veh_pwt_inner=${VEH_PWT_INNER} "
    IVARGS+=" --tow_ttc=${TOW_TTC} "
