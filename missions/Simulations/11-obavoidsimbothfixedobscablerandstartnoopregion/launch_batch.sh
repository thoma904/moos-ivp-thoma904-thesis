...
# pwt_outer and pwt_inner distances to test (lockstep pairs)
OUTER_VALS=(35 40 45 50)
INNER_VALS=(25  30 35 40)
...
# 4x4 loop: all combinations of tow pwt and vessel pwt
for ((t=0; t<${#OUTER_VALS[@]}; t++)); do
    TOW_PWT_OUTER=${OUTER_VALS[$t]}
    TOW_PWT_INNER=${INNER_VALS[$t]}

    for ((v=0; v<${#OUTER_VALS[@]}; v++)); do
        VEH_PWT_OUTER=${OUTER_VALS[$v]}
        VEH_PWT_INNER=${INNER_VALS[$v]}
...
echo "Batch complete: 16 configurations x $NUM_RUNS runs each (random start positions)"
