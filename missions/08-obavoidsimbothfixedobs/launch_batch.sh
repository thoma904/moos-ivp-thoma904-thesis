...
# Launch command with varying start positions only
for ((j=1; j<=NUM_RUNS; j++)); do
    echo "=== Run $j/$NUM_RUNS | obs=$OBSTACLE_AMT (fixed), varying start position ==="
    ...
    for ((retry=1; retry<=MAX_RETRIES; retry++)); do
        ...
        ./launch_sleep.sh --obs="$OBSTACLE_AMT" --sep="$OBSTACLE_SEP" --rand --nogui 10
        EXIT_CODE=$?
    ...
    if [ "$SUCCESS" = true ]; then
        tar -czf "${RESULTS_DIR}/obavoidsim_fixedobs_run_$(printf '%03d' $j).tar.gz" LOG_*/ XLOG_*/
    else
...
echo "Batch complete: $NUM_RUNS runs with fixed obstacles, varying start positions"
